...
## 2026-08-27: 第1層・第2層の総合報告書と関連画像だけをGit管理対象にする

- 変更前: バックテスト成果物を含む`data/`以下は一律にGit管理対象外としていた。
- 変更理由: 利用者から、第1層と第2層の総合報告書を関連画像と一緒にcommit/pushする明示的な指示があったため。報告書は検証結論を共有する文書であり、生のMT5結果とは用途が異なる。
- 決定: 第1層総合報告書1点・関連SVG 3点、第2層総合報告書1点・関連SVG 3点だけを例外として追跡する。既存の`/data/`除外規則は維持し、対象8ファイルを明示的に追加する。
- 非変更: HTML、PNG、JSON、INI、set、ログ、ティック、その他のバックテスト成果物、および報告書生成スクリプトは引き続きGit管理しない。

## 2026-08-27: 第2層総合報告書を第1層と同じ構成・視認性へ拡張

- 変更前: 第2層には目的・結論・実施条件・モード別結果を記した短い報告書があるが、第1層報告書にある3層の位置づけ、WF期間、全ケース表、グラフ、詳細な制約説明までは含まれていない。
- 変更理由: 利用者の指示により、第2層も第1層と同じように「目的、結論、詳細」の順で、表とグラフから検証範囲・再現性・解釈上の限界を追える総合報告書にするため。
- 決定: 完了済み4 suite・64件のJSONを単一の正とし、再計算可能な生成スクリプトからMarkdownとSVGを作る。No Delay・固定188msの決定論的比較と、ランダム延滞の再現性不足を視覚的にも分離する。
- 非変更: バックテストの再実行、結果JSON、WF専用set、EA、Safety Gate、既存の第1層報告書は変更しない。新しい報告書と画像は`data/slippage_tolerance`へ保存し、Git管理しない。

## 2026-08-27: スリッページ耐性検証の第2層をQQの許容偏差感応度試験として実施

- 変更前: 第1層では3EA共通でBid/Ask加工カスタムシンボルを使用し、不利約定価格・取引コストストレスを評価した。
- 変更理由: 第2層では価格データの加工ではなく、EA側の注文許容条件を分離して評価する必要がある。現在の専用setで許容偏差入力を確認できたのはQuantum Queenの`InpSlippage`だけであり、Smart Gold HunterとWave Riderには該当する公開入力がない。
- 決定: 第2層の実計算対象をQuantum QueenのWF1～WF4とする。通常XAUUSD、Deposit 3,000 USD、Every tick based on real ticks、No Delayを固定し、`InpSlippage=100`を再現基準として先に検証した後、`0 / 2 / 5 / 10 points`を実行する。合計は20バックテストとする。
- 実装方針: 既存のScenario、set処理、MT5Executor、HTML解析、deal監査を共有する。既存のWF専用setは変更せず、実行フォルダ内に`InpSlippage`だけを変更した派生setを作る。
- Safety Gate: `InpSlippage=100`は各WFの既存3,000 USD基準へ、Trades・deal系列を中心とする既存ゲートを適用する。4WFすべての基準通過後だけ他水準へ進む。
- 解釈: `InpSlippage`は最大許容偏差であり、強制的な約定差ではない。全水準が一致した場合は、No Delayテスター環境で効果を観測できなかった結果として扱う。
- 保存: 成果物は`data/slippage_tolerance`配下へ保存し、Git管理しない。

## 2026-08-27: 第2層の100-point基準でSwap会計ドリフトを限定許容

- 変更前: `InpSlippage=100`の基準は、既存のNet Profit・PF・RF・Sharpe・Equity DD許容幅を含む一般Safety Gateへ完全に合格した場合だけ通過させる計画だった。
- 実測: QQ/WF1の現在実行はNet Profit 52.96 USDとなり、既存自動基準53.91 USDから-0.95 USDずれた。一方、Trades 147、294 dealの日時・方向・約定価格ハッシュ、Commission -105.84 USD、deal Profit 179.91 USD、EA SHA-256、元set SHA-256、派生set全入力は一致した。Swapだけが-20.16 USDから-21.11 USDへ-0.95 USD変化し、Net Profit差と完全一致した。
- 変更理由: 取引ロジックと注文系列が同一で、差額が現在の銘柄Swap条件だけに限定される場合に試験を停止すると、`InpSlippage`水準間の同一環境比較を開始できないため。
- 決定: 第2層の100-point基準に限り、一般Safety Gateが会計指標で不一致でも、(1) Trades・deal件数・deal系列ハッシュが完全一致、(2) Commissionとdeal Profitが過去の自動基準と一致、(3) 過去と現在のNet ProfitがそれぞれCommission・Swap・deal Profitの合計へ整合、(4) Net Profit差が会計項目差で説明可能、をすべて満たす場合は`PASS_WITH_ACCOUNTING_DRIFT`として通過させる。
- 非変更: 取引系列が異なる場合、Commissionまたはdeal Profitが異なる場合、会計差でNet Profit差を説明できない場合は従来どおり停止する。Capital Stressなど既存キャンペーンのSafety Gateは変更しない。
- 比較基準: 100以外の水準は、過去のNet Profitではなく、同じ実行時点で得た現在の100-point結果と比較する。

## 2026-08-27: No Delayで差がないため第2層を固定188ms・ランダム延滞へ拡張

- 変更前: 第2層はまずNo Delayで`InpSlippage=100 / 0 / 2 / 5 / 10`を4WFへ適用し、差が観測されない場合に延滞条件との組み合わせを検討する計画だった。
- 実測: No Delayの20件はすべて成功し、100以外の16件でNet Profit、Trades、PF、RF、Sharpe、Equity DD、deal件数、deal系列ハッシュが100-point基準と完全一致した。
- 変更理由: No Delayでは注文要求から処理までの価格変化がなく、許容偏差の効果を観測できなかったため。利用者が過去の約定遅延耐性で使用した固定188msとランダム延滞を用い、価格変化が起き得る条件内で`InpSlippage`だけを比較する。
- 決定: 第2層追加試験として`ExecutionMode=188`（固定188ms）と`ExecutionMode=-1`（ランダム延滞）を実行する。各モード内で100 pointsを先に基準として実行し、その後0・2・5・10 pointsを比較する。その他の条件はNo Delay試験と同じとする。
- 比較方法: 延滞モード間の損益を直接比較せず、同一延滞モード内の100-point基準との差だけを評価する。
- ランダム性: ランダム延滞は100-point基準を独立に再実行し、deal系列と主要指標の再現性を確認する。再現しない場合は、単一実行同士の差を`InpSlippage`効果と断定しない。
